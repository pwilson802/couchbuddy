...
function MakeQuestionsList(slug) {
  let questions = [];
  if (slug == "random-movie-quiz") {
    questions = [
      makeTaglineQuestion,
      makeTaglineQuestion,
...
async function getAlternativeMovies(
  originalMovie,
  num,
  character = "",
  actor = ""
...
async function makeTaglineQuestion(movie, extraMovies, internalData) {
  ...
  const questionObject = makeQuestionObject(
    question,
    answers,
    alternatives,
    false
...
async function makeWhoPlayedCharacterQuestion(
  movie,
  extraMovies,
  internalData
) {
...
async function validateDirector(director, compareMovie) {
  ...
  const movies = allMovies
    .filter((item) => item.id !== compareMovie.id)
    .filter((item) => item.title !== compareMovie.title);
  let result = [];
  let count = 0;
  for (let movie of movies) {
    let crew = await getMovieCrew(movie.id);
...
async function makeWhoDidntDirectQuestion(movie, extraMovies, internalData) {
  ...
  const answerObj = await getSimilarMovieByDirector(
    director,
    directedMovies[0]
  );
...
async function makeMoviesStaringPersonQuestion(
  ...
  const alternateMovies = await getAlternativeMovies(
    correctMovies[0],
    5,
    "",
    star
