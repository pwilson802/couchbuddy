export default async function handler(req, res) {
  ...
  let url = `https://api.themoviedb.org/3/tv/${id}/videos?api_key=${TMB_KEY}&language=en-US`;
  res.setHeader(
    "Cache-Control",
    "public, s-maxage=3600, stale-while-revalidate=86400"
  );
  // A transient TMDB failure here must never 500 - the client's Promise.all
  // with the tv-detail fetch would reject and the card would spin forever,
  // since it has no error handling of its own. Degrade to "no trailer".
  try {
